 ...
 *
 * The viewport is pinned to the Visual Viewport so inputs stay above the
 * virtual keyboard on iOS Safari and WeChat WKWebView.
 */
    ...
    <DialogPrimitive.Portal>
      ...
      <DialogPrimitive.Viewport
        className={cn(
          ...
          "z-50 flex items-end justify-center overflow-hidden",
        )}
      >
        <DialogPrimitive.Popup
          className={cn(
            ...
            side === "bottom" &&
              "max-h-[min(85%,720px)] rounded-t-2xl pb-[max(1rem,env(safe-area-inset-bottom))] shadow-[var(--shadow-border),var(--shadow-lg)]",
            side === "full" &&
              "h-full rounded-none pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)]",
            className,
          )}
          {...props}
        >
